...
| Layer enforcement | ArchUnitNET, one test file (~20 lines) | Catches `using Soarscore.Infrastructure` in Domain at build; actively maintained (TNG's C# port of ArchUnit) | NetArchTest.Rules (last release 2023, effectively unmaintained) |
...
| Command/query dispatch | **Hand-rolled** `ICommandHandler`/`IQueryHandler` + `IDispatcher` over `IServiceProvider`, ~60 LOC | Zero licence risk, more inspectable than a behaviour pipeline | **MediatR — commercial licence v12+**; Wolverine (see Deliberately not used) |
...
| Logging | `ILogger` only, console/file provider | One fewer dependency; consistent with hand-rolled/in-box lean at this scale, no platform to query structured output in yet | Serilog (deferred with OpenTelemetry — trigger is the same: a platform to send to) |
...
| Assertions | AwesomeAssertions (Apache 2.0 fork of the FluentAssertions v7 API) | **FluentAssertions v8 is paid for commercial use**; fork carries an explicit community commitment to stay non-commercial, and keeps the familiar `.Should()` API | FluentAssertions v8+, Shouldly (BSD-3-Clause, safe on licence but a different API style) |
...
| Acceptance / BDD test framework | Reqnroll 3.3.4 (`Reqnroll.xunit.v3`), `tests/Soarscore.Acceptance.Tests` | The repo's first genuinely user-facing, end-to-end workflow (capture-a-score-steel-thread-plan.md WI-13) — CLAUDE.md's testing-approach section calls for Gherkin/BDD once a feature reaches this shape, and Reqnroll is the actively-maintained SpecFlow successor for it over xUnit v3. This is the one place `WebApplicationFactory` (Microsoft.AspNetCore.Mvc.Testing) is used, deliberately reversing the "Test entry point" row above for this layer only: an acceptance suite's job is to prove the HTTP surface itself, over a real Testcontainers PostgreSQL, closing gap.md §7 ("no automated end-to-end test"). Every other layer keeps `IDispatcher`. Repo's pinned xunit.v3 3.2.2 / xunit.runner.visualstudio 3.1.5 already satisfy Reqnroll's ≥3.0.2 floor | SpecFlow (Reqnroll's own predecessor — a licence-driven fork, same shape as the AwesomeAssertions/FluentAssertions split above); hand-written `[Fact]` scenarios (loses Gherkin's living-documentation value for a workflow this user-facing) |
...
| Property-based | CsCheck, for scoring-engine invariants (normalisation, rounding, drop-worst) | C#-native, runs inside a plain `[Fact]` — no separate discovery pipeline to verify against xUnit v3, unlike `FsCheck.Xunit`'s `[Property]` attribute | FsCheck (F#-first generator composition, unconfirmed xUnit v3 discovery support) |
...
| Licence hygiene | CI step: `dotnet list package --include-transitive` through **nuget-license** (sensslen), allow-list of permitted licence types, build fails outside it | Automate the constraint rather than trust it | dotnet-project-licenses (original tool, now effectively abandoned; nuget-license is its maintained successor) |
...
- **Wolverine** — same reasoning as above despite the coherent Critter Stack pairing with Marten; single-digit writes/minute (NFR) doesn't need outbox/messaging infrastructure, and the hand-rolled dispatcher already satisfies the requirement while staying fully inspectable.
