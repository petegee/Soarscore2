...
> **Status: restored history. All eight issues are resolved and the resolutions bind.**
>
> Removed from HEAD in `38cb008` alongside `scoring-service-plan.md`, restored verbatim
> at `900df6d`. The title says "Open"; none of them is. Every resolution below was
> decided and is implemented in `src/Soarscore.Domain/Scoring/` — these are the
> semantics the shipped pipeline actually has, recorded nowhere else.
>
