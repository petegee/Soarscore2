...
| jerilderie-2010 | 50-f3j.json | not-expressible | refused | G5: the club's landing tape is graduated in POINTS — the rulebook distance lookup pre-composed on the tape (scheme-3 identity lookup; scores-raw Landing 93 → GS raw 262+93=355) — while the seed declares a metres metric (landingDistance, F3J.10.5 ladder) and no catalogue seed declares a points-kind landing metric. A seed-declaration mismatch, not inexpressibility: a derived-equivalent tape-points F3J seed would run this comp. `config tests/GliderscoreFixtures/jerilderie-2010/competition.json lookups.landingSchemes[0].Name` `config tools/Soarscore.SeedData/json/50-f3j.json phases[0].tasks[0].metrics.landingDistance` `stub kanban/backlog/tape-points-landing-seeds.md` |
